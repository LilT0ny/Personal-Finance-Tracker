    ...
    <div className="space-y-4">
      ...
      <div className="card mb-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className={`w-12 h-12 rounded-full flex items-center justify-center ${type === 'income' ? 'bg-success/20' : 'bg-danger/20'
              }`}>
              {type === 'income' ? (
                          ...
                          <div className="flex items-center gap-1 shrink-0">
                            <button
                              onClick={() => {
                                ...
                                setTransactionCategory(transaction.categoria_id);
                                setNote(transaction.descripcion || '');
                              }}
      ...
      {editingTransaction && (
        <div className="fixed inset-0 z-50 flex items-end lg:items-center justify-center">
          ...
          <div className="relative w-full lg:max-w-md bg-card rounded-t-3xl lg:rounded-2xl p-6 animate-slide-up lg:animate-fade-in max-h-[90vh] overflow-y-auto">
            <button onClick={() => setEditingTransaction(null)} className="absolute top-4 right-4 p-2 tap-target rounded-full hover:bg-background">
              <X className="w-5 h-5 text-foreground-muted" />
            </button>

            <h2 className="text-xl font-bold mb-4">
              Editar {type === 'income' ? 'Ingreso' : 'Egreso'}
            </h2>
      ...
      {deletingTransaction && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => setDeletingTransaction(null)} />
          ...
          <div className="relative w-full max-w-sm bg-card rounded-2xl p-6 shadow-2xl animate-fade-in">
            ...
            <div className="bg-background rounded-xl p-4 mb-4">
              ...
              <p className={cn("text-center text-2xl font-bold", type === 'expense' ? "text-danger" : "text-success")}>
                {type === 'expense' ? '-' : '+'}${deletingTransaction.monto.toFixed(2)}
              </p>
